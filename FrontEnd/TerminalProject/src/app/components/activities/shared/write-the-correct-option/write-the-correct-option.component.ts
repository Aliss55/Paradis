...
export class WriteTheCorrectOptionComponent implements OnChanges{
  ...
  constructor( public shufflerService: ShufflerService, private themeService : ThemeSwitcherService, public notificationService : NotificationService, private cdr: ChangeDetectorRef) {
      }
  ...
  public checkAnswer(index_card:number): void {
    if(this.answer[index_card] === '' || this.answer[index_card] === undefined){
      this.notificationService.notifyError('Por favor introduce una respuesta válida');
      return;
    }
